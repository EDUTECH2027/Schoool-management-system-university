...
export default function TeacherTimetable() {
  ...
  useEffect(() => {
    api.portalTeacherTimetable().then(setSlots).catch(() => {});
  }, []);
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-x-auto">
        <table className="w-full text-sm min-w-[600px]">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
            {(periods.length ? periods : PERIODS).map(period => (
                    ...
                    <td key={day} className="px-4 py-3">
                      {slot ? (
                        <div className="bg-indigo-50 dark:bg-indigo-900/20 rounded-lg p-2">
                          <p className="font-medium text-indigo-800 dark:text-indigo-200 text-xs">{slot.subject_name}</p>
                          <p className="text-indigo-600 dark:text-indigo-400 text-xs">{slot.class_name}</p>
                          {slot.time && <p className="text-indigo-500 text-xs">{slot.time}</p>}
                        </div>
                      ) : (
